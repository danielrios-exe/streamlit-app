...
st.subheader(
    ":blue[Streamlit es un *framework* para la creación de aplicaciones web "
...
histo_selected = st.sidebar.selectbox(
    "Elección del Mes para el Histograma:", vars_mes, index=default_hist
)
st.sidebar.divider()
...
ganan_selected = st.sidebar.selectbox(
    "Elección de Persona para Mostrar las Ganancias Personales:",
    vars_per,
    index=default_pers,
)
st.sidebar.divider()
...
color_selected = st.sidebar.selectbox(
    "Paleta de Color para la Matriz de Correlación:", vars_cmap
)
...
selec_val_corr = st.sidebar.radio(
    "Valores de Correlación:", options=["Activo", "Inactivo"]
)
...
mes_multi_selected = st.sidebar.multiselect(
    "Elementos de la Matriz de Correlación:", vars_mes, default=vars_mes
)
...
st.markdown(
    ":blue[Este **DataFrame** contiene información de varias personas, "
